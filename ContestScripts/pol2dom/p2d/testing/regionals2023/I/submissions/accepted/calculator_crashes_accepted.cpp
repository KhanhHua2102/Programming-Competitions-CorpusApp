...
ostream& operator<<(ostream& s, const rational_t& r)
{
    return s << r.p << '/' << r.q;
}
...
ostream& operator<<(ostream& s, const operator_t& o)
{
    return s << o.op << o.val;
}

ostream& operator<<(ostream& s, const vector<operator_t>& v)
{
    if (v.size() > 0)
        s << v[0];
