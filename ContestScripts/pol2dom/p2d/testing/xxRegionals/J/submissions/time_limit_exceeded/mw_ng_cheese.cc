// This solution is O(n log^2 n + G) where G is the sum of the sizes of grundy number sets at all states.
// It also uses a simple cheese to short circuit and say Jay wins if there is only one subtree.
#include <bits/stdc++.h>
using namespace std;
...
int f(int at) {
    ...
    // Simulate iterating through all grundy numbers
    tr[at].fill_all();
    return mex;
}
